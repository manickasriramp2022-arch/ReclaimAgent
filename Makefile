...
.DEFAULT_GOAL := help
.PHONY: help venv install demo generate run report replay verify queue test lint types check fmt clean distclean ci
...
demo: install ## Generate, run, verify, report, open (the whole pitch, offline)
	@echo "=== 1/4  generating a synthetic test-mode batch ==="
	@$(BIN)/reclaim generate --seed $(SEED) --size $(SIZE)
	@echo ""
	@echo "=== 2/4  running the recovery pipeline and the naive baseline ==="
	@$(BIN)/reclaim run --batch $(BATCH) --no-llm
	@echo ""
	@echo "=== 3/4  verifying the audit trail ==="
	@$(BIN)/reclaim verify-audit
	@echo ""
	@echo "=== 4/4  rendering the report ==="
...
ci: check ## Everything CI runs, including verify-audit on a fixture run
